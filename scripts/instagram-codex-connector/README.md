...
| `backfill-all-media.ps1` | 계정의 전체 게시물을 훑어 성과(조회/도달/저장/공유)를 한 번에 분석해 `media-archive.json`을 채움 (월간 리포트를 계정 개설 이후 전체 기간으로 만들고 싶을 때 1회 실행) |
| `backfill-follower-history.ps1` | 계정 레벨 "일별 팔로워 순증감" 인사이트로 과거 날짜별 팔로워 수를 역산해 `history.json`을 채움 (Meta가 실제로 보관 중인 기간만큼만 복원됨) |
| `1_연결설정.bat` / `2_인사이트수집.bat` / `3_아침브리프_자동등록.bat` / `4_결과보기.bat` / `5_바탕화면_바로가기_만들기.bat` / `6_노션연동설정.bat` / `7_전체피드분석.bat` | 위 스크립트들을 더블클릭만으로 실행하기 위한 실행기 |

## 계정 전체 기간 피드 분석 (선택)

팔로워 "수"의 과거 기록은 Instagram API가 소급 제공하지 않아 오늘 이후부터만 쌓이지만,
**게시물별 성과(조회/도달/저장/공유)는 오래된 게시물도 지금 조회할 수 있습니다.**
`7_전체피드분석.bat`을 실행하면 계정에 있는 모든 게시물을 훑어서 `media-archive.json`에
채워 넣고, 이후 "월간 리포트" 탭에 계정 개설 이후 전체 기간의 발행 수/평균 도달·저장률/
TOP3 게시물이 표시됩니다.

게시물이 많으면 시간이 오래 걸리고 API 호출 한도에 걸릴 수 있습니다. 한도에 걸리면
지금까지 분석한 내용을 저장하고 안내 메시지와 함께 멈추므로, 나중에(예: 1시간 뒤) 다시
실행하면 이어서 진행됩니다.

## 과거 팔로워 수 복원 (선택, 결과 보장 안 됨)

`8_팔로워기록복원.bat`은 Instagram 계정 레벨 인사이트의 "일별 팔로워 순증감"
(`follower_count`) 지표를 이용해, 오늘 팔로워 수에서 거꾸로 계산하여 과거 날짜별
팔로워 수를 복원해 `history.json`에 채워 넣습니다. 기본적으로 180일 전까지 시도합니다.

**Meta가 이 지표를 실제로 며칠/몇 달치나 보관하는지는 계정마다 다르고 미리 알 수
없습니다.** 실행해보면 결과 화면에 "가장 오래된 복원 날짜"가 나오는데, 원하는 만큼
못 가져왔다면 그 이상은 Meta 쪽에서 데이터를 제공하지 않는다는 뜻입니다.
이미 실제로 수집된(라이브) 날짜는 덮어쓰지 않습니다.
